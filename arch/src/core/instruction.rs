...
impl Instruction {
    pub fn execute_unhandled(&mut self, cpu: &mut CPU) -> InstructionResult {
        match self.operation {
            ...
            Operation::Add => instructions::add(&self.operands, cpu),
            Operation::Sub => instructions::sub(&self.operands, cpu),
            Operation::Mul => instructions::mul(&self.operands, cpu),
            Operation::Div => instructions::div(&self.operands, cpu),
            Operation::Mod => instructions::mod_(&self.operands, cpu),
            Operation::Sqt => instructions::sqt(&self.operands, cpu),
            Operation::Cbt => instructions::cbt(&self.operands, cpu),
            Operation::Sqr => instructions::sqr(&self.operands, cpu),
            Operation::Cbe => instructions::cbe(&self.operands, cpu),
            Operation::Min => instructions::min(&self.operands, cpu),
            Operation::Max => instructions::max(&self.operands, cpu),
            Operation::Adc => instructions::adc(&self.operands, cpu),
            Operation::Sbc => instructions::sbc(&self.operands, cpu),
            Operation::Asr => instructions::asr(&mut self.operands, cpu),

            // 0x02?? - Logic Instructions
            Operation::And => instructions::and(&self.operands, cpu),
            Operation::Or => instructions::or(&self.operands, cpu),
            Operation::Xor => instructions::xor(&self.operands, cpu),
            Operation::Nor => instructions::nor(&self.operands, cpu),
            Operation::Nad => instructions::nad(&self.operands, cpu),
            Operation::Imp => instructions::imp(&self.operands, cpu),
            ...
            Operation::Ldr => instructions::ldr(&mut self.operands, cpu),
            Operation::Str => instructions::str(&mut self.operands, cpu),
